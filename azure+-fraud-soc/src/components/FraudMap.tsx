...
const initialMarkers: MapMarker[] = [
  { id: '1', coordinates: [-74.006, 40.7128], severity: 'critical', city: 'New York' },
  ...
  { id: '5', coordinates: [-46.6333, -23.5505], severity: 'critical', city: 'São Paulo' },
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const newMarker: MapMarker = {
        ...
        severity: Math.random() > 0.8 ? 'critical' : Math.random() > 0.6 ? 'warning' : 'safe',
        city: 'Roaming Event'
      };
    ...
    <div className="bg-[#0F172A] rounded border border-slate-800 overflow-hidden flex flex-col h-full shadow-2xl relative">
      <div className="absolute top-4 left-4 z-10">
        <h3 className="text-[10px] font-bold text-white uppercase tracking-widest leading-none flex items-center gap-2">
          ...
          Global Fraud Map <span className="font-normal text-slate-500">— Live Events</span>
        </h3>
      </div>
          ...
          {markers.map(({ id, coordinates, severity }) => (
            <Marker key={id} coordinates={coordinates}>
              <motion.circle
                initial={{ r: 0 }}
                animate={{ 
                  r: severity === 'critical' ? [4, 8, 4] : 3,
                }}
                transition={{ duration: 2, repeat: Infinity }}
                fill={severity === 'critical' ? '#F43F5E' : severity === 'warning' ? '#F59E0B' : '#10B981'}
                stroke="white"
                strokeWidth={0.5}
              />
              {severity === 'critical' && (
      ...
      <div className="absolute bottom-4 right-4 z-10 bg-slate-900/80 backdrop-blur-md p-2 rounded border border-slate-700/50 text-[8px] uppercase font-bold tracking-tighter text-slate-400 space-y-1">
        ...
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span> Suspicious
        </div>
        <div className="flex items-center gap-2">
